...
|----|--------|--------|------------|
| `dashboard` | 5 | **Дашборд** | Единый компактный экран (две колонки, без прокрутки): отмечаете галочками нужные операции → кнопка **«Выполнить выбранное»** (все выполняются последовательно, вывод — в общий журнал с отметками времени) |
| `roles` | 10 | Роли и компоненты | **Полный список ролей/компонентов с сервера** (`Get-WindowsFeature`): галочки, группировка (сначала роли, затем компоненты), поиск, статус установки, панель описания справа + `Install-WindowsFeature` |

**Дашборд** объединяет типовые операции одной кнопкой:

| Группа | Операции |
|--------|----------|
| Безопасность | отключить брандмауэр · отключить IE ESC · отключить Windows Defender · DEP: только основные программы и службы |
| Удалённый доступ | включить RDP · включить WinRM / PS Remoting |
| Система | активация Windows (ключ по версии: 2022 / 2025) · энергосхема «максимальная производительность» |
| Профили | перенос профилей на `D:\Users` (галочка активна только при наличии диска `D:`) |
| Софт | установка 7-Zip, Chrome, PuTTY, WinSCP, Notepad++, Radmin VPN, VMware Tools (winget, иначе прямые MSI/EXE; уже установленное пропускается) |
...
Операции RDS берут параметры из сохранённых настроек (`activation`, `licensing`, `rdsPolicy`).
...
## Меню установки всех ролей и компонентов

Раздел **Роли и компоненты** запрашивает с сервера `Get-WindowsFeature` — то есть
**все** доступные роли и компоненты Windows Server (не вручную из списка). Это
позволяет выбрать что угодно из текущего набора компонентов сервера:

* список формируется на лету из командлета `Get-WindowsFeature`;
* группировка по типу: **Роли** и **Компоненты**;
* колонки: отмечено/нет (галочка), имя (код, например `RDS-Licensing`),
  статус «установлено / не установлено»;
* живой **поиск** по имени/коду и кнопки «Выбрать всё» / «Снять всё»;
* клик по строке — отметить/снять; **Установить выбранные** запускает
  `Install-WindowsFeature`.

`roles.json` для этого раздела больше не нужен (список берётся с сервера).
...
├─ roles.json            # (необязательно) резервный список ролей; раздел «Роли» берёт их с сервера
├─ roles.example.json    # примеры ролей (для справки)
...
├─ LICENSE               # MIT
├─ README.md
└─ modules/              # разделы (см. выше): dashboard, roles
```
