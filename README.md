...
|----|--------|--------|------------|
| `dashboard` | 5 | **Дашборд** | Единый экран: отмечаете галочками нужные операции → кнопка **«Выполнить выбранное»** (все выполняются последовательно, вывод — в общий журнал) |
| `roles` | 10 | Роли и компоненты | **Полный список ролей/компонентов с сервера** (`Get-WindowsFeature`): галочки, группировка (роли/компоненты), поиск, статус установки + `Install-WindowsFeature` |
...
| Группа | Операции |
|--------|----------|
| Безопасность | отключить брандмауэр · отключить IE ESC · отключить Windows Defender · выключить DEP |
...
| Софт | установка 7-Zip, Chrome, PuTTY, WinSCP (winget, иначе прямые MSI/EXE) |
| Мониторинг | Zabbix Agent 2 — через прокси / напрямую (MSI 7.4.14, PSK, хост через API) |

Ключи активации Windows (GVLK) зашиты в `activate_os.ps1`:
**2022** → `WX4NM-KYWYW-QJJR4-XV3QB-6VM33`, **2025** → `D764K-2NDRG-47T6Q-P8T8W-YP6DF`.
...
├─ LICENSE               # MIT
├─ README.md
└─ modules/              # разделы (см. выше): dashboard, roles, rds, security
```
